...
chess = chess[['winner', 'white_rating', 'black_rating', 'opening_ply']]
...
chess.loc[(chess['white_rating'] >= chess['black_rating'] - 200) & (chess['white_rating'] <= chess['black_rating'] + 200), 'fair'] = 1
chess.loc[(chess['white_rating'] < chess['black_rating'] - 200) | (chess['white_rating'] > chess['black_rating'] + 200), 'fair'] = 0

chess.loc[chess['winner'] == 'white', 'white_win'] = 1
chess.loc[chess['winner'] != 'white', 'white_win'] = 0

fair = chess[chess['fair'] == 1]
# print(len(fair)) 
# 13736


print(f"\033[93m{pd.crosstab(index=fair['rate_cat'], columns=fair['white_win'], margins=True)}")
joint = pd.crosstab(index=fair['rate_cat'], columns=fair['white_win'])
...
ax = sns.barplot(x='rate_cat', y='white_win', data=fair, ci=None)
